...
namespace NovaUI.Controls
{
	...
	public class NovaVSlider : Control
	{
		...
		{
			_knob.Y = Math.Max(Width / 2, Math.Min(e.Y - _offsetY, Height - (Width / 2) - 1));
			double height = Height - Width;
			double knobY = _knob.Y - (Width / 2);
		...
		private void CalculateKnobPosition()
		{
			...
			_knob.Y = Math.Max((Width / 2) + 1, Math.Min((int)((num / den * height) + (Width / 2)), Height - (Width / 2) - 1));
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			_mouseDown = true;
			_canDrag = true;
			_offsetY = e.Y - _knob.Y;

			int cX = _knob.X + (_knob.Width / 2);
			int cY = _knob.Y;
			int radius = Width / 2;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (_knobStyle == SliderKnobStyle.SolidKnob)
				e.Graphics.FillEllipse((_mouseHover ? (_mouseDown ? _dragColor : _knobColor.Lighter(0.1f)) : _knobColor).ToBrush(),
					_knob.X - 1, _knob.Y - (_knob.Height / 2), _knob.Width, _knob.Height);
			...
			if (_showValue)
			{
				...
				e.Graphics.DrawString(_value.ToString(), new Font(Font.Name, (float)(_knob.Height / 3f)), ForeColor.ToBrush(),
					new Rectangle(_knob.X - 1, _knob.Y - (_knob.Height / 2) + (_knob.Height % 2 == 0 ? 0 : 1), _knob.Width, _knob.Height), Constants.CenterAlign);
			}
		}
	}
}
