	...
	public class NovaRadioButton : RadioButton
	{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (_borderRadius > 0)
			{
				...
				e.Graphics.FillPath((Checked ? (_mouseHover ? _radioColor.Lighter(0.1f).Darker(_mouseDown ? 0.1f : 0) : _radioColor) : _borderColor).ToBrush(), new Rectangle(0, 0, Height - 1, Height - 1).Roundify(_borderRadius));
			...
			{
				e.Graphics.FillRectangle((Checked ? (_mouseHover ? _radioColor.Lighter(0.1f).Darker(_mouseDown ? 0.1f : 0) : _radioColor) : _borderColor).ToBrush(), 0, 0, Height, Height);
